	...
	class Nepali_Calendar
	{
		...
		public function ENG_TO_NEP_TIME($English_date = NULL){
			...
			if ($nepali_time_data) {
				...
				if ($day >=1) {
					$this->NEP_DATE_TIME($English_date);
				}
		...
		public function ENG_TIME_DATE($english_date){
			$scatt_date = $this->SeperatData($english_date, "-");
			$year= $scatt_date[0];
			$month = $scatt_date[1];
			$day = $scatt_date[2];
			$m = $this->_get_english_month($month); 
			return $date_time = $m. " ".$day." , ".$year;
		...
		public function NEP_DATE_TIME($english_date = NULL){
			...
			if ($scatt_date) {
				...
				if ($dateOnly) {
					...
					echo ("".$nepali_date['day']. ", ".$nepali_date['nmonth']. " ".$np_date." ".$np_year."");
					// return $args ;
				}
